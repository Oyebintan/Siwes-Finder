...
export default async function JobDetails({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8 animate-fade-in-up">
      <div>
        <Link href="/student/jobs" className="inline-flex items-center gap-2 text-sm text-gray-500 dark:text-gray-400 hover:text-accent-600 dark:hover:text-accent-300 transition-colors mb-6 font-medium">
        ...
        <div className="bg-surface-1 border border-surface-border shadow-sm rounded-3xl p-8 relative overflow-hidden">
          <div className="relative z-10 flex flex-col md:flex-row md:items-start justify-between gap-6">
            <div>
              <div className="w-16 h-16 bg-accent-50 dark:bg-accent-500/10 rounded-2xl flex items-center justify-center mb-6 border border-accent-100 dark:border-accent-500/20">
                <Building2 className="w-8 h-8 text-accent-600 dark:text-accent-300" />
              </div>
            ...
            <div className="bg-gray-50 dark:bg-surface-2 p-6 rounded-2xl border border-gray-100 dark:border-surface-border min-w-[200px] text-center shrink-0">
              ...
              <p className="text-2xl font-bold text-accent-600 dark:text-accent-200">
                {job.stipend || 'Unpaid'}
              </p>
            </div>
          </div>
        ...
        <div className="lg:col-span-2 space-y-8">
          <section className="bg-surface-1 border border-surface-border shadow-sm rounded-3xl p-8">
            <h2 className="text-xl font-bold mb-4 flex items-center gap-2 text-gray-900 dark:text-white">
              <span className="w-1.5 h-6 bg-accent-500 rounded-full inline-block"></span>
              Job Description
            </h2>
          ...
          <section className="bg-surface-1 border border-surface-border shadow-sm rounded-3xl p-8">
            <h2 className="text-xl font-bold mb-4 flex items-center gap-2 text-gray-900 dark:text-white">
              <span className="w-1.5 h-6 bg-accent-300 rounded-full inline-block"></span>
              Requirements
            </h2>
            <ul className="space-y-3">
              {job.requirements.map((req: string, idx: number) => (
                <li key={idx} className="flex items-start gap-3 text-gray-600 dark:text-gray-300">
                  <div className="w-6 h-6 rounded-full bg-accent-50 dark:bg-accent-500/10 text-accent-600 dark:text-accent-300 flex items-center justify-center shrink-0 text-xs font-bold mt-0.5 border border-accent-100 dark:border-accent-500/20">
                    {idx + 1}
                  </div>
