          ...
          {applications.map((app: any) => (
            <div key={app._id} className="bg-surface-1 border border-surface-border hover:border-accent-400/40 shadow-sm rounded-2xl p-6 flex flex-col md:flex-row md:items-center justify-between gap-6 transition-all">
              ...
              <div className="flex items-center gap-4">
                {app.status === 'Pending' && (
                  <span className="flex items-center gap-1.5 px-4 py-2 rounded-full bg-amber-50 dark:bg-amber-500/10 text-amber-600 dark:text-amber-400 border border-amber-200 dark:border-amber-800/40 font-bold text-sm">
                    <Clock className="w-4 h-4" /> Pending Review
                  </span>
                )}
                {app.status === 'Accepted' && (
                  <span className="flex items-center gap-1.5 px-4 py-2 rounded-full bg-emerald-50 dark:bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 border border-emerald-200 dark:border-emerald-800/40 font-bold text-sm">
                    <CheckCircle className="w-4 h-4" /> Accepted
                  </span>
                )}
                {app.status === 'Rejected' && (
                  <span className="flex items-center gap-1.5 px-4 py-2 rounded-full bg-red-50 dark:bg-red-500/10 text-red-600 dark:text-red-400 border border-red-200 dark:border-red-800/40 font-bold text-sm">
