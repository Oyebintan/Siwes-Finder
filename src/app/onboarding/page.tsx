      ...
      <div className="w-full max-w-2xl animate-fade-in-up">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <button onClick={() => handleRoleSelection('student')} disabled={loading}
            className="group relative p-8 rounded-3xl bg-surface-1 border border-surface-border hover:border-brand-400 hover:shadow-md transition-all text-left disabled:opacity-50">
            <div className="w-16 h-16 rounded-2xl bg-brand-50 dark:bg-brand-500/10 text-brand-600 dark:text-brand-300 flex items-center justify-center mb-6 group-hover:scale-110 transition-transform">
            ...
            {loading && <Loader2 className="absolute top-8 right-8 w-6 h-6 animate-spin text-brand-500" />}
          </button>
