...
// to placements and posting opportunities are gated server-side (see
// POST /api/applications, POST /api/jobs); this banner is the visible
// explanation for why those actions might get rejected. Dismissible for
// the tab session so it doesn't nag on every click, but reappears on the
// next full page load until the account is actually verified.
export function EmailVerificationBanner({ email }: EmailVerificationBannerProps) {
  ...
  return (
    <div className="mb-5 flex items-center gap-3 px-4 py-3 rounded-xl bg-warning-bg border border-warning/20 text-[13px]">
      ...
      <p className="flex-1 font-medium text-warning">
        Verify your email to apply to placements and post opportunities.
      </p>
      <Link
